...
$assignedClientId = (int) ($case['client_id'] ?? 0);
$assignedLawyerId = (int) ($case['lawyer_id'] ?? 0);

// Admins can view any case. Clients and lawyers may only view their own assigned cases.
if ($userRole !== 'admin') {
    $isAssignedClient = ($loggedInUserId === $assignedClientId);
    $isAssignedLawyer = ($loggedInUserId === $assignedLawyerId);

    if (!$isAssignedClient && !$isAssignedLawyer) {
        http_response_code(403);
        exit('Access denied');
    }
}

$pageTitle = $userRole === 'admin' ? 'Case Summary | Admin' : 'Case Details | Zimba & Partners';
$assetPathPrefix = '../';
